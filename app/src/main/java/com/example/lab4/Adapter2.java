...
public class Adapter2 extends RecyclerView.Adapter<ViewHolder> {
    ...
    Adapter2(Context context , BibDatabase database) {
        this.database = database;
        this.inflater = LayoutInflater.from(context);
        this.dataSize = database.size();
    }
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder , int position) {
        int pos = position % dataSize;
        BibEntry curEntry = database.getEntry(pos);
    ...
    public int getItemViewType(int position) {
        int pos = position % dataSize;
        switch (database.getEntry(pos).getType()) {
            case BOOK: return 2;
